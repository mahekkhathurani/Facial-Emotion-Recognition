...
# emotions will be displayed on your face from the webcam feed
    ...
    while True:
        ...
        for (x, y, w, h) in faces:
            ...
            cv2.putText(frame, emotion_dict[maxindex], (x + 20, y - 60), cv2.FONT_HERSHEY_SIMPLEX, 1, (7, 109, 16),
                        2, cv2.LINE_AA)
            ...
            emoji_face = emoji_faces[np.argmax(prediction[0])]
            # emoji_face=int(emoji_face)
            for index, emotion in emotion_dict.items():
                text = str(
                    round(float(prediction[0][index] / total_sum * 100), 2)) + "%"
                # for drawing progress bar
                cv2.rectangle(frame, (100, index * 20 + 10),
                              (100 + int(prediction[0][index] * 100), (index + 1) * 20 + 4),
                              (255, 0, 0), -1)
                # for putting emotion labels
                cv2.putText(frame, emotion, (10, index * 20 + 20),
                            cv2.FONT_HERSHEY_SIMPLEX, 0.5, (7, 109, 16), 2)
                # for putting percentage confidence
                cv2.putText(frame, text, (105 + int(prediction[0][index] * 100), index * 20 + 20),
